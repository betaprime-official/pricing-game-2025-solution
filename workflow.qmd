...
**📋 First time setup:** Copy your team's `p{X}_loss_data.csv` and `p{X}_rating_structure.json` files from `wds/` to `files/` directory before running this chunk.
...
print(f"📊 Player {player_num} detected")
```
...
print(f"✅ Loaded {len(df):,} records with {len(df.columns)} columns from {loss_data_file}")
df.head()
```
...
print("\n💡 Edit 'outputs/bands_freq.json' to customize bands:")
...
print(f"\n✅ Frequency model: {len(freq_bands)} variables configured")
print(f"📊 Banded columns: {banded_vars}")
```
...
print("Frequency Model Relativities (exp(β)):")
...
if selected_vars_freq:
    ...
    print("ℹ️ No variables selected for frequency model - no relativities to display")
```
...
print("\n💡 Edit 'outputs/bands_sev.json' to customize bands:")
print("   - Numeric: Change cutpoints (e.g., [25, 35, 50, 65])")
print("   - Categorical: Group levels by nesting (e.g., [[\"Mercedes\"], [\"Toyota\", \"Ford\"]])")
print("   - Then re-run the next chunk to apply changes")
```
...
print(f"\n✅ Severity model: {len(sev_bands)} variables configured")
print(f"📊 Banded columns: {banded_vars}")
```
...
# Display the relativities table
print("Severity Model Relativities (exp(β)):")
...
if selected_vars_sev:
    ...
    print("ℹ️ No variables selected for severity model - no relativities to display")
```
...
print(f"🗑️ Removing frequency band columns: {freq_band_cols}")
df_for_severity = df_for_severity.drop(columns=freq_band_cols)

# Apply banding
df_for_severity, _ = apply_banding_config(df_for_severity, sev_config)

print(f"📊 Severity dataset ready with {len([col for col in df_for_severity.columns if col.endswith('_band')])} band columns")
```
...
# Combined pure premium = frequency RATE × severity RATE
...
print(f"\n💡 Consolidated config: {len(consolidated_config)} variables (from {len(selected_vars_freq)} freq + {len(selected_vars_sev)} sev)")
...
print(f"\n✅ Consolidated model: {len(all_selected_vars)} variables (union of freq + sev)")
print(f"📊 Freq selected: {selected_vars_freq}")
print(f"📊 Sev selected: {selected_vars_sev}")
print(f"📊 Consolidated variables: {all_selected_vars}")
```
...
print(f"\n✅ Rating structure saved to: {rating_structure_path}")
print(f"📁 Player {player_num} solution ready for submission")
```
...
# Load price caps configuration if available
price_caps_config = None
price_caps_path = 'files/price_caps.json'
if os.path.exists(price_caps_path):
    with open(price_caps_path, 'r') as f:
        price_caps_config = json.load(f)
    print(f"✅ Loaded price caps from {price_caps_path}")
else:
    print(f"ℹ️  No price caps found (not used in this game)")
...
df_inforce['new_comm_premium'] = calculate_means_with_factors(df_inforce, new_rating_structure, price_caps_config=price_caps_config)
...
print(f"\n📝 Rating structure saved as: {rating_structure_path}")
print(f"🔄 Player {player_num} solution ready for submission.")
print(f"💡 To modify pricing strategy, edit the JSON file and re-run impact analysis.")
```